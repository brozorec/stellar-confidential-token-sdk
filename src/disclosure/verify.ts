/**
 * Disclosure-receiver verification — the mandatory receiver protocol for the
 * D-recipient and D-sender circuits.
 *
 * The trust-boundary rule is load-bearing here: the ONLY bundle values
 ...
 * PVK is read from is dictated by the variant and the event payload
 ...
 * the bundle would prove nothing.
 */
...
/** Which receiver-protocol step a rejection happened at. */
...
export interface VerifiedDisclosure {
  /** The disclosed amount — trustworthy as the chain itself. */
  amount: bigint;
...
export async function verifyDisclosure(params: {
  ...
  /**
   * Pinned verification key (the vk.json in `circuits/disclosure/` matching
   * the bundle's circuit_id). When given, the VK derived from the loaded
   * circuit bytecode must match byte-for-byte — this makes the "circuit_id →
   * audited circuit" agreement checkable.
   */
  pinnedVk?: Uint8Array;
  /**
  ...
  // Pin the verification key before trusting any verify result.
  if (pinnedVk) {
  ...
  // Step 1 — resolve ref_E to exactly one token-contract event.
  ...
  // Step 2 — account lookups dictated by the variant + the EVENT payload.
  ...
  // Step 3 — auxiliary state: addr_f recomputed from the contract address.
  const addrF = addressToField(client.cfg.contracts.token);

  // Step 4 — public inputs, in the circuit's exact order. Bundle values
  ...
  // Step 6 — decrypt the sealed value.
